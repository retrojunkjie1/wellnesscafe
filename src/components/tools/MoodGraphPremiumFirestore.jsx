...
const SEED_ROWS = [
  ...
  },
  {
    ...
    score: 8,
    intensity: 8,
    note: "Connected with loved ones",
...
export default function MoodGraphPremiumFirestore() {
  ...
  const stats = useMemo(() => {
    ...
    return {
      avgScore: (totalScore / rows.length).toFixed(1),
      topMood,
  ...
  const freqData = useMemo(() => {
    const counts = MOODS.reduce((m, o) => {
      m[o.key] = 0;
      return m;
    }, {});
    rows.forEach((r) => {
      if (counts[r.mood] != null) counts[r.mood] += 1;
    });
  ...
  return (
    <div className="relative min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 text-white overflow-hidden">
      ...
      <div className="fixed inset-0 z-0 pointer-events-none">
        ...
        <svg
          className="absolute top-0 w-full h-96 opacity-20"
        ...
        >
          ...
          <path
            fill="#fbbf24"
            fillOpacity="0.15"
            d="M0,64L48,69.3C96,75,192,85,288,122.7C384,160,480,224,576,245.3C672,267,768,245,864,208C960,171,1056,117,1152,85.3C1248,53,1344,43,1392,37.3L1440,32L1440,0L1392,0C1344,0,1248,0,1152,0C1056,0,960,0,864,0C768,0,672,0,576,0,480,0,384,0,288,0,192,0,96,0,48,0L0,0Z"
          />
        </svg>
      </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400">
                Track emotions • Note triggers • See patterns
              </p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-12">
          <div className="lg:col-span-2 bg-gradient-to-br from-slate-800/50 to-slate-900/50 rounded-3xl border border-amber-400/30 p-8 backdrop-blur overflow-hidden group">
            ...
            <div className="relative space-y-8">
              <div>
                <h2 className="text-lg font-bold text-amber-300 flex items-center gap-2 mb-6">
                  <Sparkles size={20} /> How are you feeling?
                </h2>
                  ...
                  {MOODS.map((m) => (
                    <button
                      ...
                      className={`p-3 rounded-xl border-2 transition-all hover:scale-110 ${
                        mood.key === m.key
                          ? `border-amber-400 bg-gradient-to-br ${m.color} shadow-lg shadow-amber-500/50`
              ...
              <div>
                <label className="text-sm font-semibold text-amber-300 block mb-3">
                  What influenced this?
                </label>
                <textarea
                  value={note}
                  onChange={(e) => setNote(e.target.value)}
                  placeholder="Describe what led to this mood. Be specific and honest..."
                  className="w-full h-32 bg-black/40 border border-white/10 rounded-xl p-4 outline-none focus:ring-2 focus:ring-amber-400/50 text-white placeholder-gray-500 resize-none"
                />
              </div>
            </div>
          </div>
          ...
          <div className="bg-gradient-to-br from-slate-800/50 to-slate-900/50 rounded-3xl border border-amber-400/30 p-8 backdrop-blur space-y-6">
              ...
              <div className="flex flex-wrap gap-2">
                {TRIGGERS.map((t) => (
                  <button
                    key={t}
                    onClick={() => toggleTag(t)}
                    className={`text-xs px-3 py-2 rounded-lg font-semibold border-2 transition-all ${
                      tags.includes(t)
                        ? "bg-gradient-to-r from-amber-500/30 to-yellow-500/30 border-amber-400 text-amber-200"
            ...
            <div>
              ...
              <button
                onClick={save}
                disabled={saving}
                className="w-full px-6 py-4 rounded-lg bg-gradient-to-r from-amber-500 to-yellow-500 hover:from-amber-600 hover:to-yellow-600 text-black text-sm font-bold transition-all hover:scale-105 shadow-lg disabled:opacity-50 flex items-center justify-center gap-2"
              >
                <Save size={18} />
            ...
            <div className="bg-amber-500/10 border border-amber-400/30 rounded-xl p-4 text-sm text-amber-100">
              ...
              <p className="text-xs">
                Regular mood tracking helps identify patterns and triggers for
        ...
        {rows.length > 0 && (
          <div>
            <h2 className="text-2xl font-bold text-amber-300 flex items-center gap-2 mb-8">
              <TrendingUp size={24} /> Analytics & Patterns
            </h2>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-12">
              <div className="bg-gradient-to-br from-slate-800/50 to-slate-900/50 rounded-2xl border border-amber-400/30 p-8 backdrop-blur">
                <h3 className="text-lg font-bold text-amber-300 flex items-center gap-2 mb-6">
                  <TrendingUp size={20} /> 14-Day Mood Trend
                </h3>
                <div className="h-72">
              ...
              <div className="bg-gradient-to-br from-slate-800/50 to-slate-900/50 rounded-2xl border border-amber-400/30 p-8 backdrop-blur">
                <h3 className="text-lg font-bold text-amber-300 flex items-center gap-2 mb-6">
                  <BarChart3 size={20} /> Mood Frequency
                </h3>
                <div className="h-72">
                  <ResponsiveContainer width="100%" height="100%">
            ...
            <div className="mt-12 bg-gradient-to-br from-slate-800/50 to-slate-900/50 rounded-2xl border border-amber-400/30 p-8 backdrop-blur">
              ...
              <div className="space-y-3 max-h-96 overflow-y-auto">
                {rows.slice(0, 12).map((entry, idx) => (
                  <div
                    key={idx}
                    className="p-4 bg-black/30 border border-white/10 rounded-xl hover:border-amber-400/30 transition-all"
                  >
                    ...
                    {entry.tags && entry.tags.length > 0 && (
                      <div className="flex flex-wrap gap-2">
                        {entry.tags.map((tag, tidx) => (
                          <span
                            key={tidx}
                            className="text-xs px-2 py-1 rounded-md bg-amber-500/20 text-amber-200"
                          >
                            {tag}
                          </span>
                        ))}
...
function buildWeeklyInsight(rows) {
  ...
  if (last7.length === 0)
    return {
      message: "No entries yet. Track your mood to unlock insights.",
      tips: [],
    };
  ...
  const tips = [
    topTrigger !== "—"
      ? `Plan a 5-minute buffer before known "${topTrigger}" contexts (box-breath 4-4-4-4).`
      : `Add a few tags to expose repeat triggers.`,
    avgScore < 6
      ? `Aim for two "restore" reps tomorrow (walk + hydrate).`
      : `Maintain momentum with a 10-minute gratitude or nature block.`,
