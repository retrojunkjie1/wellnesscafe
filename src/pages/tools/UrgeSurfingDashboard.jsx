...
const WaveBackground = () => (
  <div className="absolute inset-0 overflow-hidden pointer-events-none">
    <svg
      className="absolute bottom-0 w-[200%] h-64 opacity-30"
    ...
    >
      ...
      <path
        fill="#f4c27a"
        fillOpacity="0.25"
        d="M0,192L60,181.3C120,171,240,149,360,160C480,171,600,213,720,229.3C840,245,960,235,1080,213.3C1200,192,1320,160,1380,144L1440,128L1440,320L1380,320C1320,320,1200,320,1080,320C960,320,840,320,720,320C600,320,480,320,360,320C240,320,120,320,60,320L0,320Z"
      />
    </svg>
    <svg
      className="absolute bottom-0 w-[200%] h-72 opacity-20"
      viewBox="0 0 1440 320"
      preserveAspectRatio="none"
    >
      <animate
      ...
      <path
        fill="#ffffff"
        fillOpacity="0.15"
        d="M0,64L48,90.7C96,117,192,171,288,186.7C384,203,480,181,576,176C672,171,768,181,864,192C960,203,1056,213,1152,224C1248,235,1344,245,1392,250.7L1440,256L1440,320L1392,320C1344,320,1248,320,1152,320C1056,320,960,320,864,320C768,320,672,320,576,320C480,320,384,320,288,320C192,320,96,320,48,320L0,320Z"
      />
    </svg>
  </div>
);
...
export default function UrgeSurfingDashboard() {
  ...
  useEffect(() => {
    ...
    if (elapsed % 5 === 0 && elapsed > 0)
      setSamples((arr) => [...arr, { t: elapsed, intensity }]);
  ...
  useEffect(() => {
    ...
    const fetchAudio = async () => {
      ...
      try {
        const url = await getDownloadURL(
          ref(storage, "urge-surfing/guide.mp3")
        );
        setAudioURL(url);
      } catch (e) {
    ...
    const fetchSessions = async () => {
      ...
      try {
        const q = query(
          collection(db, "urgeSurfingSessions"),
      ...
      sessions
        ...
        .map((s, i) => ({
          ...
          intensity: Math.round(
            (s.samples?.reduce((a, b) => a + b.intensity, 0) || 0) /
              (s.samples?.length || 1)
          ),
        })),
    [sessions]
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400">
                Navigate cravings with mindfulness & presence
              </p>
            </div>
          </div>
          <button
            onClick={() => setSoundEnabled(!soundEnabled)}
            className="p-2 rounded-lg hover:bg-orange-500/20 transition"
          >
      ...
      <div className="max-w-7xl mx-auto px-6 py-12">
          ...
          <div className="lg:col-span-2 bg-gradient-to-br from-slate-800/50 to-slate-900/50 rounded-3xl border border-orange-500/30 p-8 backdrop-blur group relative overflow-hidden">
            ...
            <div className="flex flex-col items-center py-12">
              <div className="relative mb-8 w-48 h-48 flex items-center justify-center">
                <div
                  className={`absolute w-48 h-48 border-4 border-orange-500/30 rounded-full transition-all duration-3000 ${breathSize}`}
                />
                <div
                  className={`absolute w-40 h-40 border-2 border-orange-400/50 rounded-full transition-all duration-3000 ${
                    breathSize === "scale-100" ? "scale-90" : "scale-100"
                  }`}
                />
              ...
              {running && (
                <div className="mb-8 text-sm font-semibold text-gray-300 capitalize">
              ...
              <div className="flex gap-4 flex-wrap justify-center mb-8">
                {!running ? (
                  <button
                    onClick={start}
                    className="px-8 py-3 bg-gradient-to-r from-green-500 to-emerald-600 rounded-lg font-semibold flex items-center gap-2 shadow hover:scale-105 transition"
                  >
                    <Play size={20} />
                    Start
                  </button>
                  ...
                  <button
                    onClick={stop}
                    className="px-8 py-3 bg-gradient-to-r from-red-600 to-red-700 rounded-lg font-semibold flex items-center gap-2 shadow"
                  >
                    <Pause size={20} />
                    Stop
                  </button>
                ...
                {elapsed > 0 && (
                  <button
                    onClick={reset}
                    className="px-6 py-3 bg-slate-700/50 hover:bg-slate-600 rounded-lg font-semibold flex items-center gap-2"
                  >
                    <RefreshCw size={18} />
                    Reset
                  </button>
              ...
              <div className="w-full">
                ...
                <input
                  ...
                  disabled={!running}
                  className="w-full h-3 bg-slate-700 rounded-lg accent-orange-500 disabled:opacity-50"
                />
              </div>
              <div className="mt-8 w-full">
                <p className="text-xs font-semibold text-orange-400 mb-3">
                  Live Intensity Heatmap (5s intervals)
                </p>
                  ...
                  {Array.from({ length: 60 }).map((_, i) => {
                    ...
                    return (
                      <div
                        ...
                        style={{ background: `rgba(244,194,122,${alpha})` }}
                      />
                    );
                  })}
                </div>
        ...
        <div className="bg-gradient-to-br from-orange-500/10 to-amber-500/5 rounded-2xl border border-orange-500/30 p-8 mb-12 backdrop-blur">
          <div className="flex justify-between mb-4">
            <div>
              <h2 className="text-xl font-bold text-orange-400 flex items-center gap-2 mb-1">
                <Zap size={20} />
                Today's Affirmation
              </h2>
              <p className="text-sm text-gray-400">
                Renew your mindset with intention
              </p>
            </div>
            <button
              onClick={() =>
          ...
          <div className="bg-gradient-to-br from-slate-800/50 to-slate-900/50 rounded-2xl border border-orange-500/30 p-6 backdrop-blur">
            ...
            {audioURL ? (
              ...
              <p className="text-xs text-gray-500 italic">
                Upload <code>urge-surfing/guide.mp3</code> to Firebase Storage
                to enable playback.
              </p>
            )}
          </div>
          ...
          <div className="bg-gradient-to-br from-slate-800/50 to-slate-900/50 rounded-2xl border border-orange-500/30 p-6 backdrop-blur">
            ...
            <div className="space-y-2 max-h-80 overflow-y-auto">
              {sessions.map((s, i) => (
                <div
                  key={i}
                  className="flex justify-between p-4 bg-slate-700/20 rounded-lg hover:bg-slate-700/40"
                >
                  <div>
                    <p className="text-sm font-semibold">{s.affirmation}</p>
                  </div>
                  <div className="text-right text-sm">
                    <p className="font-bold text-orange-400">
                      {secondsToClock(s.duration)}
                    </p>
                  </div>
                </div>
              ))}
