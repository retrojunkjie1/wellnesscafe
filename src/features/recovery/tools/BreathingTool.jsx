...
// Speech helper (browser TTS) - Enhanced with voice selection
const speak = (text, priority = false, selectedVoice = null) => {
  try {
    if (priority) {
...
const makeRecommendation = ({
  ...
  if (inhale < 4 || exhale < 4) {
    tips.push(
      "Try longer, slower breaths (4-6 seconds) for deeper relaxation."
    );
  }
  if (exhale < inhale) {
    tips.push(
      "Make exhale longer than inhale to activate the parasympathetic nervous system."
    );
  }
  if (hold > 0 && hold > inhale) {
    tips.push(
      "Consider reducing hold time - focus on smooth inhale/exhale flow."
    );
  }
  ...
  if (lengthSec < 120) {
    tips.push(
      "Longer sessions (3-5 minutes) often provide more noticeable benefits."
    );
  }
  ...
  if (delta > 0) {
    tips.push(
      "Great progress! Continue with this pattern and gradually increase duration."
    );
  }
  ...
  if (inhale === exhale && hold === 0) {
    tips.push(
      "Box breathing (equal inhale/hold/exhale/pause) is excellent for focus."
    );
  }

  const summary = `Session: ${cycles} cycles @ ${inhale}-${hold}-${exhale}s. Mood change: ${
    delta > 0 ? `+${delta}` : delta
  }. Duration: ${Math.round(lengthSec / 60)}min ${lengthSec % 60}s.`;
...
const BreathingTool = ({
  ...
  // Wrapper for speak function with selected voice
  const speakWithVoice = (text, priority = false) => {
    if (voiceEnabled) {
      speak(text, priority, selectedVoice);
    }
  };
  ...
  const radius = useMemo(() => {
    ...
    const total =
      phase === Phase.Inhale ? inhale : phase === Phase.Hold ? hold : exhale;
    const progress = total > 0 ? 1 - secondsLeft / total : 0;

    if (phase === Phase.Inhale) {
      return 40 + (maxRadius - 40) * progress;
    }
    if (phase === Phase.Hold) {
      return maxRadius;
    }
    if (phase === Phase.Exhale) {
      return maxRadius - (maxRadius - 40) * progress;
    }
    return 40;
  ...
  const nextPhase = () => {
    if (phase === Phase.Idle) {
      ...
      speakWithVoice("Inhale slowly", true);
      return;
    }
    if (phase === Phase.Inhale) {
      ...
      speakWithVoice(
        hold > 0 ? "Hold your breath comfortably" : "Exhale slowly",
        true
      );
      return;
    }
    if (phase === Phase.Hold) {
      ...
      speakWithVoice("Exhale slowly", true);
      return;
    }
    if (phase === Phase.Exhale) {
  ...
  const startSession = () => {
    ...
    speakWithVoice("Starting breathing session. Inhale slowly.", true);
  };

  const pauseSession = () => {
    setRunning(false);
    speakWithVoice("Paused", true);
  };
  ...
  const stopSession = () => {
    ...
    setSecondsLeft(0);
    setEndedAt(new Date());
    speakWithVoice("Session stopped", true);
  };
  ...
  const resetSession = () => {
    ...
    setShowMoodAfter(false);
    setLastRecommendation(null);
    speakWithVoice("Session reset", true);
  };
  ...
  useEffect(() => {
    const completed =
      !running && startedAt && endedAt && phase === Phase.Complete;
    if (!completed) return;
  ...
  const canResume = useMemo(
    () => !running && phase !== Phase.Idle && phase !== Phase.Complete,
    [running, phase]
  );
  ...
  const canReset = useMemo(
    () => phase === Phase.Complete || (!running && phase !== Phase.Idle),
    [running, phase]
  );
  ...
  const instructionText = useMemo(() => {
    if (phase === Phase.Idle)
      return "Configure your breathing pattern and click Start";
  ...
  return (
    <div className="breathing-tool">
        ...
        <div className="voice-controls">
          ...
          {voiceEnabled && availableVoices.length > 0 && (
            <div className="voice-selector-group">
              <label className="voice-selector-label">Voice:</label>
              <select
          ...
          >
            <defs>
              <radialGradient id="breathingGradient" cx="50%" cy="50%" r="50%">
                ...
                <stop offset="100%" stopColor="rgba(212, 180, 131, 0.1)" />
              </radialGradient>
              <radialGradient id="inhaleGradient" cx="50%" cy="50%" r="50%">
                <stop offset="0%" stopColor="rgba(212, 180, 131, 0.6)" />
                <stop offset="100%" stopColor="rgba(212, 180, 131, 0.2)" />
              </radialGradient>
              <radialGradient id="holdGradient" cx="50%" cy="50%" r="50%">
                <stop offset="0%" stopColor="rgba(203, 180, 255, 0.6)" />
                <stop offset="100%" stopColor="rgba(203, 180, 255, 0.2)" />
              </radialGradient>
              <radialGradient id="exhaleGradient" cx="50%" cy="50%" r="50%">
                <stop offset="0%" stopColor="rgba(100, 181, 246, 0.6)" />
                <stop offset="100%" stopColor="rgba(100, 181, 246, 0.2)" />
              </radialGradient>
              <radialGradient id="completeGradient" cx="50%" cy="50%" r="50%">
                <stop offset="0%" stopColor="rgba(129, 199, 132, 0.6)" />
                <stop offset="100%" stopColor="rgba(129, 199, 132, 0.2)" />
              </radialGradient>
            </defs>
            ...
            />
            <text x="160" y="140" className="breathing-text phase-text">
              {phaseLabel}
            </text>
            ...
            <text x="160" y="190" className="breathing-text cycle-text">
              {phase !== Phase.Idle
                ? `Cycle ${Math.min(currentCycle + 1, cycles)} / ${cycles}`
                : ""}
            </text>
          </svg>
        </div>
      ...
      <div className="breathing-controls">
        {canStart && (
          <button onClick={startSession} className="breathing-btn btn-primary">
            Start Session
          </button>
        )}
        {canPause && (
          <button
            ...
            className="breathing-btn btn-secondary"
          >
            Pause
          </button>
        )}
        {canResume && (
          <button onClick={resumeSession} className="breathing-btn btn-success">
            Resume
          </button>
        )}
        {canStop && (
          <button onClick={stopSession} className="breathing-btn btn-danger">
            Stop
          </button>
        )}
        {canReset && (
          <button onClick={resetSession} className="breathing-btn btn-outline">
            Reset
          </button>
        )}
      </div>
      ...
      {voiceEnabled && (
        <div className="voice-notice">
          <strong>Voice Guidance:</strong> Make sure your device volume is on.
          Voice instructions will guide your breathing.
        </div>
      )}
