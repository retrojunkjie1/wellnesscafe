...
const ProviderDirectory = () => {
  ...
  useEffect(() => {
    const fetchProviders = async () => {
      if (!db) {
        console.warn(
          "Firebase not initialized - provider directory unavailable"
        );
        return;
      }
      try {
  ...
  const filtered = providers.filter(
    (p) =>
      ...
      (p.serviceTypes?.join(" ").toLowerCase() || "").includes(
        search.toLowerCase()
      )
  );
    ...
    <section className="pv-wrap">
      <div className="pv-header-row">
        <div>
          ...
          <p className="pv-sub">
            Browse verified providers. Create an account to unlock full profiles
            and booking.
          </p>
        </div>
        <div className="pv-cta-right">
          ...
          {!user && (
            <button className="pv-solid" onClick={() => navigate("/signin")}>
              Sign In
            </button>
          )}
        </div>
