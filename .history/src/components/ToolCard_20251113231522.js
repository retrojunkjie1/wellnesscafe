...
export default function ToolCard({
  ...
  const content = (
    <div
      className={`relative h-full rounded-2xl border border-orange-500/20 p-6 backdrop-blur overflow-hidden group transition-all duration-300 ${
        isLocked
      ...
      <div className="relative z-10 flex flex-col h-full">
        ...
        <div className="flex items-center justify-between mt-4 pt-4 border-t border-orange-500/10 group-hover:border-orange-400/30 transition-colors">
          ...
          {isLocked ? (
            ...
            <ChevronRight
              size={16}
              className="text-gray-400 group-hover:text-orange-400 transition-all group-hover:translate-x-1"
            />
          )}
        </div>
      </div>
